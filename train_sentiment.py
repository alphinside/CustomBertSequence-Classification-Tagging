...
from utils_kemenkunham_sentiment import (compute_metrics, convert_examples_to_features,
...
def train(args, train_dataset, model, tokenizer):
    ...
    # target = np.array([sample[-1].cpu().numpy()
    ...
    #     samples_weight, len(samples_weight)) if args.local_rank == -1 else DistributedSampler(train_dataset)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if result_f1 > best_result:
                        ...
                        best_checkpoint = 'checkpoint-{}'.format(global_step)
...
def main():
    ...
    # Prepare GLUE task
    args.task_name = 'kemenkunham_sentiment'
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        model = model_class.from_pretrained(
            os.path.join(args.output_dir, 'checkpoint-250'))
        global_step = 'checkpoint-250'.split(
            '-')[-1] if len('checkpoint-250') > 1 else ""
        tokenizer = tokenizer_class.from_pretrained(args.output_dir)
        model.to(args.device)
